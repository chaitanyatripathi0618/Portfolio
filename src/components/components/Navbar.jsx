...
const Navbar = () => {
  ...
  return (
    <nav className={`${style.paddingX} w-full flex items-center py-5 top-0 z-20 bg-primary`}>
      <div className='w-full flex justify-between items-center max-w-7xl mx-auto'>
        ...
        >
          ...
          <p className='text-white text-[18px] font-bold cursor-pointer'>
            Chaitanya <span className='sm:block hidden'>| Web Developer</span>
          </p>
        </Link>
        ...
        <div className='sm:hidden flex flex-1 justify-end items-center'>
          ...
          {navLink.map((link) => (
            <li key={link.id} className={`${active=== link.title ? "text-white":" text-secondary"} font-medium cursor-pointer font-poppins text-[16px]`} 
            onClick={()=>{
              setToggle(!toggle)
              setActive(link.title)
            }}>
